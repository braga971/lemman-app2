...
serve(async (req) => {
  ...
  try{
    ...
    const { bucket, path, expiresIn = 3600 } = payload || {}
    if (!bucket || !path){
      return new Response(JSON.stringify({ error: 'bucket and path required' }), { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders } })
    }
    const { data, error } = await supaAdmin.storage.from(bucket).createSignedUrl(path, expiresIn)
    if (error){
      return new Response(JSON.stringify({ error: error.message }), { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders } })
    }
    return new Response(JSON.stringify({ ok: true, signedUrl: data.signedUrl }), { headers: { 'Content-Type': 'application/json', ...corsHeaders } })
  }catch(e){
    console.error('get-signed-url internal error', e)
